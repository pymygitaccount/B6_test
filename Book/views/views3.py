...
from Book.views.view1 import view_a
...
from Book.forms import AddressForm, StudentForm, BookForm
...
def home_view(request): # Funtion based view
    if request.method == "POST":
        print(request.POST)
        ...
        if form.is_valid():
            ...
            messages.info(request, 'Rediredting to the home page')  # <-
        ...
        return render(request, "form_home.html", context)
         ...
         HttpResponse("Invalid HTTP method", status=405)
...
class CBV_TemplateView(RedirectView):
